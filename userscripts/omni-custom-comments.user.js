...
(function () {
  ...
  function placeButton() {
    ...
    const paymentLabel = pageElements('label, div, span, td, th')
      .filter(visible)
      .find(element => /^payment\s+type$/i.test(clean(element.textContent)));
    const addPayment = pageElements('button, input, a, [role="button"]')
      .filter(visible)
      .find(element => /^add\s+a\s+new\s+payment$/i.test(clean(element.value || element.textContent)));
    let paymentBlock = paymentLabel?.parentElement;
    while (paymentBlock && paymentBlock !== document.body) {
      if (addPayment && paymentBlock.contains(addPayment) && paymentBlock.querySelectorAll('input, select, textarea').length >= 5) break;
      paymentBlock = paymentBlock.parentElement;
    }
    if (paymentBlock && paymentBlock !== document.body) paymentBlock.style.transform = 'translateY(16px)';
