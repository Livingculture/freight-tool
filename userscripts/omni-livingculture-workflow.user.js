...
// @downloadURL  https://raw.githubusercontent.com/Livingculture/freight-tool/main/userscripts/omni-livingculture-workflow.user.js
// @updateURL    https://raw.githubusercontent.com/Livingculture/freight-tool/main/userscripts/omni-livingculture-workflow.user.js
// ==/UserScript==
...
(function () {
  ...
  const HUBSPOT_LEAD_SOURCE_FALLBACK = {
    label: 'Leads Source',
    options: [
      ['S/R', 'Walk in'], ['Call', 'Call'], ['Email', 'Email'], ['Ticket', 'Ticket'],
      ['Repeat customer', 'Repeat customer'], ['Referral customer', 'Referral customer'],
      ['AKL Homeshow', 'AKL Homeshow'], ['Waikato Homeshow', 'Waikato Homeshow'],
      ['Canterbury Homeshow', 'Canterbury Homeshow'], ['Fieldays Exhibition', 'Fieldays Exhibition']
  ...
  function leftmostOmniSaveButton() {
    const labels = ['Save As Draft', 'Save', 'Save & Email', 'Save Back to List', 'Save to Admin', 'Approve Back to List', 'Approve'];
    return labels
        ...
        onload: (response) => {
          try {
            ...
            if (response.status >= 200 && response.status < 300 && data.ok) {
              ...
              try { localStorage.setItem(HUBSPOT_LEAD_SOURCE_CACHE_KEY, JSON.stringify(result)); } catch (error) {}
              resolve(result);
              return;
            }
  ...
  function cachedHubSpotLeadSourceOptions() {
    try {
      const cached = JSON.parse(localStorage.getItem(HUBSPOT_LEAD_SOURCE_CACHE_KEY) || 'null');
      if (cached?.label && Array.isArray(cached.options) && cached.options.length) return cached;
    } catch (error) {}
    return HUBSPOT_LEAD_SOURCE_FALLBACK;
  }
  ...
  async function chooseHubSpotLeadSource() {
    ...
    let leadSourceData = cachedHubSpotLeadSourceOptions();
    const overlay = document.createElement('div');
    ...
    overlay.innerHTML = `
      <div class="lc-hs-lead-source-panel">
        ...
        <select id="lcHsLeadSourceSelect"><option value="">Choose lead source...</option>${leadSourceData.options.map(option => `<option value="${escapeHtml(option.value)}">${escapeHtml(option.label)}</option>`).join('')}</select>
        <div class="lc-hs-lead-source-actions">
    ...
    overlay.querySelector('label').textContent = leadSourceData.label;
    const select = overlay.querySelector('#lcHsLeadSourceSelect');
    const continueButton = overlay.querySelector('#lcHsLeadSourceContinue');
    select.focus();
    void fetchHubSpotLeadSourceOptions().then((fresh) => {
      if (!overlay.isConnected || !fresh.options.length) return;
      const previousValue = select.value;
      leadSourceData = fresh;
      overlay.querySelector('label').textContent = fresh.label;
      select.innerHTML = `<option value="">Choose lead source...</option>${fresh.options.map(option => `<option value="${escapeHtml(option.value)}">${escapeHtml(option.label)}</option>`).join('')}`;
      if (fresh.options.some(option => option.value === previousValue)) select.value = previousValue;
    }).catch(() => {});
    const chosen = new Promise((resolve) => continueButton.addEventListener('click', () => {
        const selected = leadSourceData.options.find(option => option.value === clean(select.value));
        if (!selected) { window.alert('Choose a lead source before continuing.'); return; }
        closeHubSpotLeadSourceModal();
        resolve(selected);
      }));
    return Promise.race([cancelled, chosen]);
  }
  ...
  async function showOmniPhotoEditor(photo, albumId, refreshAlbum) {
    ...
    const toolButton = (text, value) => { const button = document.createElement('button'); button.type = 'button'; button.textContent = text; button.style.cssText = 'padding:8px 11px;border:1px solid #9eb7bc;border-radius:6px;background:#fff;color:#183f3a;font-weight:700;cursor:pointer;'; button.onclick = () => { labelBox?.remove(); labelBox = null; draft = null; tool = value; [...tools.querySelectorAll('[data-tool]')].forEach((item) => { item.style.background = '#fff'; item.style.color = '#183f3a'; }); button.style.background = '#09a8bc'; button.style.color = '#fff'; canvas.style.cursor = value === 'select' ? 'move' : 'crosshair'; paint(); updateSave(); }; button.dataset.tool = value; tools.appendChild(button); return button; };
    ...
    function draw(mark, isSelected) {
      ...
      if (mark.tool === 'text') { const label = mark.label || 'Note', size = 24 * scale, padding = 8 * scale; context.font = `bold ${size}px Arial`; const width = context.measureText(label).width; context.fillStyle = 'rgba(255,255,255,.9)'; context.fillRect(mark.x1 - padding, mark.y1 - size - padding, width + padding * 2, size + padding * 2); context.fillStyle = mark.color; context.textAlign = 'left'; context.textBaseline = 'alphabetic'; context.fillText(label, mark.x1, mark.y1); if (isSelected) { context.strokeStyle = mark.color; context.lineWidth = 3 * scale; context.setLineDash([8 * scale, 5 * scale]); context.strokeRect(mark.x1 - padding, mark.y1 - size - padding, width + padding * 2, size + padding * 2); } context.restore(); return; }
    ...
    function askLabel(mark, clientX, clientY) {
      document.querySelectorAll('[data-lc-measurement-entry]').forEach((entry) => entry.remove());
      labelBox?.remove();
      draft = null;
      const rect = canvas.getBoundingClientRect();
      const x = clientX ?? rect.left + ((mark.x1 + mark.x2) / 2 / canvas.width) * rect.width;
      const y = clientY ?? rect.top + ((mark.y1 + mark.y2) / 2 / canvas.height) * rect.height;
      const entry = document.createElement('form');
      entry.dataset.lcMeasurementEntry = '1';
      entry.style.cssText = `position:fixed;z-index:2147483647;left:${Math.min(innerWidth - 190, Math.max(10, x + 12))}px;top:${Math.min(innerHeight - 60, Math.max(10, y - 20))}px;display:flex;gap:4px;width:180px;padding:5px;background:#fff;border:2px solid #09a8bc;border-radius:7px;box-shadow:0 4px 16px #0007;`;
      labelBox = entry;
      const input = document.createElement('input'); input.placeholder = mark.tool === 'text' ? 'Type note' : 'e.g. 2400 mm'; input.style.cssText = 'width:112px;min-width:0;border:0;outline:0;padding:5px;font-weight:700;';
      const yes = document.createElement('button'); yes.type = 'submit'; yes.textContent = '✓';
      const no = document.createElement('button'); no.type = 'button'; no.textContent = '×';
      [yes, no].forEach((button) => button.style.cssText = 'width:25px;border:0;border-radius:4px;background:#063b78;color:#fff;font-weight:800;');
      const closeEntry = () => { entry.remove(); if (labelBox === entry) labelBox = null; paint(); updateSave(); };
      entry.onsubmit = (event) => { event.preventDefault(); marks.push({ ...mark, label: input.value.trim() || (mark.tool === 'text' ? 'Note' : 'Measurement') }); closeEntry(); };
      no.onclick = closeEntry;
      input.onkeydown = (event) => { if (event.key === 'Escape') closeEntry(); };
      entry.append(input, yes, no); document.body.appendChild(entry); input.focus(); updateSave();
    }
  ...
  async function showCustomerAlbumsPopup(button) {
    button.disabled = true;
    button.textContent = 'Loading…';
    try {
      const result = await customerPhotosRequest({ url: CUSTOMER_PHOTOS_API_URL });
      const albums = Array.isArray(result.albums) ? result.albums : [];
      closeCustomerAlbumPopup();
      const overlay = document.createElement('div');
      overlay.id = 'lc-omni-customer-album-popup';
      overlay.style.cssText = 'position:fixed;inset:0;z-index:2147483647;display:flex;align-items:center;justify-content:center;padding:24px;background:rgba(12,29,54,.55);font-family:Arial,sans-serif;box-sizing:border-box;';
      ...
      const render = () => {
        const term = clean(search.value).toLowerCase(); body.replaceChildren();
        const visible = albums.filter((album) => !term || [album.customer_name, album.customer_email, ...(album.quote_numbers || []), ...(album.job_numbers || [])].some((value) => clean(value).toLowerCase().includes(term)));
        visible.forEach((album) => {
          const card = document.createElement('button'); card.type = 'button'; card.style.cssText = 'display:grid;grid-template-columns:92px minmax(0,1fr);gap:12px;align-items:center;text-align:left;padding:10px;border:1px solid #d5e1e5;border-radius:10px;background:#fff;color:#183f3a;cursor:pointer;';
          const preview = document.createElement('div'); preview.style.cssText = 'width:92px;height:78px;border-radius:7px;background:#edf3f6;overflow:hidden;display:flex;align-items:center;justify-content:center;color:#647a77;font-size:12px;';
          const firstPhoto = (album.photos || []).find((photo) => photo.url);
          if (firstPhoto) { const image = document.createElement('img'); image.src = firstPhoto.url; image.alt = ''; image.style.cssText = 'width:100%;height:100%;object-fit:cover;'; preview.appendChild(image); } else preview.textContent = 'No photos';
          const details = document.createElement('div');
          ...
          card.onclick = () => {
            ...
            if (!(album.photos || []).length) { const empty = document.createElement('p'); empty.textContent = 'No photos in this album.'; body.appendChild(empty); }
          };
          body.appendChild(card);
        });
        if (!visible.length) { const empty = document.createElement('p'); empty.textContent = albums.length ? 'No albums match this search.' : 'You do not have any customer photo albums yet.'; empty.style.cssText = 'grid-column:1/-1;padding:30px;text-align:center;color:#647a77;'; body.appendChild(empty); }
      };
      search.oninput = render; header.append(heading, close); panel.append(header, search, body); overlay.appendChild(panel); overlay.onclick = (event) => { if (event.target === overlay) closeCustomerAlbumPopup(); }; document.body.appendChild(overlay); render();
    } catch (error) { window.alert(error?.message || 'The customer albums could not be loaded.'); }
    finally { button.disabled = false; button.textContent = 'View Albums'; }
  }
  ...
  function handleActionButtonClick(button) {
    ...
    try {
      ...
      if (button.id === VIEW_CUSTOMER_ALBUMS_BUTTON_ID) {
        void showCustomerAlbumsPopup(button);
        return;
      }
  ...
  function addCustomerPhotosButton() {
    ...
    let viewAlbumsButton = document.getElementById(VIEW_CUSTOMER_ALBUMS_BUTTON_ID);
    if (!viewAlbumsButton) {
      viewAlbumsButton = document.createElement('button');
    ...
    viewAlbumsButton.style.display = 'inline-flex';
    viewAlbumsButton.style.alignItems = 'center';
    viewAlbumsButton.style.justifyContent = 'center';
    viewAlbumsButton.style.position = 'static';
