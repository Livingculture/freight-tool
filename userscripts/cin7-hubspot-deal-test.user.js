...
// @connect      workflow-git-hubspot-order-deal-association-test-steves-projects.vercel.app
// @connect      *.vercel.app
...
(function () {
  ...
  const BUTTON_ID = 'lc-site-visit-inline-button-test-disabled';
  const HUBSPOT_BUTTON_ID = 'lc-hubspot-deal-test-inline-button-v1';
  const QUOTE_REVIEW_BUTTON_ID = 'lc-quote-review-inline-button-test-disabled';
  const ACTION_ROW_ID = 'lc-cin7-action-row-test-disabled';
  const FLOATING_BAR_ID = 'lc-cin7-floating-actions-test-disabled';
  ...
  function submitHubSpotDeal(button) {
    ...
    if (!window.confirm(`Create HubSpot TEST deal?\n\nThis sends to the Workflow test branch only.\n\n${summary || 'Cin7 sale details will be sent to HubSpot.'}`)) {
      return;
    }
    ...
    GM_xmlhttpRequest({
      ...
      onload: (response) => {
        ...
        if (response.status >= 200 && response.status < 300 && data.ok) {
          ...
          const message = data.duplicate
            ? `HubSpot TEST deal already exists:\n${data.dealName || data.dealId}`
            : `HubSpot TEST deal created:\n${data.dealName || data.dealId}`;
          if (data.hubspotUrl) {
      ...
      onerror: () => {
        button.disabled = false;
        button.textContent = originalText;
        window.alert('Could not connect to the HubSpot TEST workflow API.');
      }
    });
  }
  ...
  function addHubSpotButton() {
    ...
    const liveHubSpotButton = document.getElementById(LIVE_HUBSPOT_BUTTON_ID) || findButtonByLabel('HubSpot Deal');
    const siteVisitButton = document.getElementById(BUTTON_ID);
    const installAnchor = findButtonByLabel('Install Fees') || findButtonByLabel('Scan');
    const commentsAnchor = findCommentsAnchor();
    const anchor = liveHubSpotButton || siteVisitButton || installAnchor || commentsAnchor;
    if (!anchor) return;
    ...
    button.type = 'button';
    button.textContent = 'HubSpot Deal TEST';
    styleInlineButton(button, '#7c3aed');
    button.style.height = `${Math.max(34, rect.height || 34)}px`;
    wireActionButton(button);
